...
import { generateQuestionsForRequirements } from "../src/services/generation/questionGenerator";
...
async function main() {
  const extraction = await extractRequirements(SAMPLE_JD);
  if (!extraction.ok) {
  ...
  const result = await generateQuestionsForRequirements(extraction.requirements, SAMPLE_RESEARCH_CONTEXT);
